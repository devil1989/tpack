...
var tpack = module.exports = {
     ...
     * @type {String}
     */
    get server() { return tpack.builder.server; },
    
    /**
     * 设置当前生成器使用的默认服务器地址。
     * @type {String}
     */
    set server(value) { tpack.builder.server = value; },
    ...
    openServer: function (options) {
        var server = tpack.startServer(options);
        if (server) {
            require('child_process').exec("start " + server.url, function (error, stdout, stderr) {
                if (error) {
                    tpack.error(stderr);
                } else {
                    tpack.log(stdout);
    ...
    plugin: function (name) {
        try {
            ...
            return function (file) {
                file.addError("Plugin “" + name + "” is not installed correctly. Nothing changed for “" + file.name + "”. Using “npm install " + name + " -g” to reinstall the plugin.", e);
            };
        }
    },
    ...
    task: function (taskName, taskAction) {
        ...
        // 执行任务。
        tpack.newSession();
        var preTask = tpack.currentTask;
        try {
            tpack.currentTask = taskName;
            return tpack.tasks[taskName].call(tpack, taskAction == null ? {} : taskAction);
        } finally {
            tpack.currentTask = preTask;
