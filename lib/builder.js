...
Builder.prototype.walk = function (callback) {
    var builder = this;
    var destName = this.getName(this.destPath);
    IO.walkDir(this.srcPath, function (name, isDir) {

        // 自动忽略目标文件夹。
        // 判断路径是否被忽略。
        if (name.indexOf(destName) === 0 && builder._ignoredFilter(name) !== null) {
            return false;
        }
...
Builder.prototype.ignored = function (path) {
    return path.indexOf(this.destPath) === 0 || this._ignoredFilter(this.getName(path)) !== null;
};
...
require("util").inherits(BuildFile, require("events").EventEmitter);
...
/**
 * 获取配置中的 sourceMap 的配置。
 * @param {} file 
 * @param {} options 
 * @returns {} 
 */
BuildFile.prototype.getSourceMapOptions = function (options) {
    var sourceMap = options.sourceMap || this.builder.sourceMap || this.sourceMapOptions;
    if (!sourceMap) {
        return;
    }
    ...
    // sourceMap: true/路径/完整配置

    this.sourceMapOptions = sourceMap = sourceMap === true ? {} : typeof sourceMap === "string" ? { path: sourceMap } : { __proto__: sourceMap };
    sourceMap.path = this.resolvePath((sourceMap.path || "$0.map").replace(/$0/g, Path.basename(this.srcPath)));
    return sourceMap;
};
 ...
 * @returns {} 
 */
BuildFile.prototype.saveSourceMap = function (sourceMap) {
    if (!sourceMap) return;

    // 自动生成 sourceMap 配置。
    if (!this.sourceMapOptions) {
        this.sourceMapOptions = {};
        this.getSourceMapOptions();
    }

    this.sourceMapOptions.data = typeof sourceMap === "string" ? JSON.parse(sourceMap) : sourceMap;

    this.on('save', function () {
        var path = this.sourceMapOptions.path;
        var data = this.sourceMapOptions.data;
        data.sources = data.sources.map(function (source) {
            return Path.relative(Path.dirname(path), source === "?" ? this.srcPath : source).replace(/\\/g, '/');
        }, this);
        this.builder.createFile(path, JSON.stringify(data)).save();
    });
 ...
 */
BuildRule.prototype.match = function (name) {
    if (this._ignoreFilter && this._ignoreFilter(name) !== null) {
        return null;
    }
    return this._srcPathFilter(name, this.destPath);
};
...
exports.isSpecifiedPath = isSpecifiedPath;
