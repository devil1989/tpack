...
*/\r\n\r\n", title, message, path) + this.content;
     ...
     * 在文件真正保存前才替换回相对路径。
     */
    createPathPlaceholder: function (name) {

        // 追加占位符处理程序。
        if (!this.hasPlaceholdeer) {
            this.hasPlaceholdeer = true;
            this.on('save', function () {
                var file = this;
                this.content = this.content.replace(/tpack:\/\/(.*?)\?/g, function(all, name) {
