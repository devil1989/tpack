...
Builder.prototype = {
     ...
     */
    newContext: function () {
        this.errors.length = this.warnings.length = this.files.length = 0;
    ...
    reloadFiles: function(){
        ...
        this.walk(function (name) {
            files.push(this.createFile(name));
        });
    },
    ...
    addFile: function (name, content) {
        var file = this.createFile('#' + this.files.length, content);
        file.name = name;
     ...
     */
    getFiles: function () {
        this.fileLoaded || this.reloadFiles();
        if (arguments.length === 0) {
            var result = [];
            for (var i = 0; i < this.files.length; i++) {
                result.push(this.files[i].name);
            }
            return result;
        }
