...
Builder.prototype = {
    ...
    /**
     * 使用当前设置的规则生成当前项目。
     * @param {Array} [filter] 要生成的文件名称过滤器。可以是通配字符串、正则表达式、函数、空(表示不符合任何条件)组合的数组。
     * @param {Boolean} [clean] 指示是否在生成前清理文件夹。默认为自动清理。
     * @returns {Number} 返回本次最终生成的文件数。 
     */
    build: function (filter, clean) {
        ...
        // 过滤器。
        if (filter) {
            filter = createNameFilter(filter);
        }

        // 清理目标文件夹。
        if (clean == null ? !filter && !containsDir(this.destPath, this.srcPath) : clean) {
    ...
    /**
     * 监听当前项目的改动并实时生成。
     * @param {Array} [filter] 要监听的文件名称过滤器。可以是通配字符串、正则表达式、函数、空(表示不符合任何条件)组合的数组。
     * @returns {Watcher} 返回监听器。
     */
    watch: function (filter) {

        this.currentAction = "watch";
    ...
    /**
     * 启用可在响应时自动处理当前规则的服务器。
     * @param {String|Number|Object} options 服务器地址或端口。 
     * @returns {HttpServer} 返回服务器对象。
     */
    startServer: function (options) {

        this.currentAction = "server";

        var serverOptions = {};

        if (typeof options == "number") {
            serverOptions.port = options;
            options = serverOptions;
        } else if (typeof options == "string") {
            serverOptions.url = options;
            options = serverOptions;
        } else if (!options) {
            options = serverOptions;
        }
        ...
        var builder = this;
        var serverConigs = require('./server.js');

        serverOptions.url = options.url || this.server;
        serverOptions.physicalPath = options.physicalPath || this.srcPath;
        ...
        var server = new (require('aspserver').HttpServer)(serverOptions);

        server.on('start', function () {
            builder.info("> Server Running At {url}", this);
        });

        server.on('stop', function () {
            builder.info("> Server Stopped At {url}", this);
        });
        ...
        if (this._changed || this.hasPlaceholder) {
            ...
            try {
                ...
                if (this.builder.onError) {
                    this.builder.onError(this, "Write Error");
                }
                return false;
            }
            this.builder.savedFileCount++;
            this.emit('save', name);
            if (this.builder.onSuccess) {
                this.builder.onSuccess(this, destPath, name);
            }
            return true;
        }
...
*/\r\n\r\n", title, message, path) + this.content;
        ...
        if (this.builder.onError) {
            this.builder.onError(this, title, error, message, path, line, column, code);
        }
    ...
    /**
     * 解析当前文件内的路径所表示的实际路径。
     * @param {String} path 要处理的相对路径。如 ../a.js
     */
    resolvePath: function (path) {
        ...
        if (/^\//.test(path)) {
            ...
            for (var vp in this.builder.virtualPaths) {
                if (pl.startsWith(vp)) {
                    return Path.resolve(this.builder.virtualPaths[vp], path.substr(vp.length));
                }
            }
            return Path.join(this.builder.srcPath + path);
        }

        return Path.join(this.srcPath, "..", path);
    },
