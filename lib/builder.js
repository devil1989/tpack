...
Builder.prototype = {
    ...
    processFileWithRule: function (file, rule) {
        ...
        // 输出处理日志。
        //this.log(!file.destName ? "> D {srcName}" : !file.srcName ? "> A {destName}" : file.srcName === file.destName ? "> M {srcName}" : "> M {srcName} => {destName}", file);
    },
    ...
    addFile: function (name, content) {
        var file = this.files[this._fileId++] = this.createFile("", content);
        file.name = name;
        return file;
    },
    ...
    build: function (clean) {
        ...
        for (var rules = this.rules, i = 0; i < rules.length; i++) {
            ...
            // 一次性规则仅在主流程执行。
            if (rule.runOnce) {
                var file = files[this._fileId++] = this.createFile("", "");
                this.processFileWithRule(file, rule);
                continue;
            }
...
BuildFile.prototype = {
    ...
    save: function (name) {
        ...
        // 如果目标名称为 null，表示删除文件。
        if (!name) {
            //this.srcName && this.builder.log("> D {0}", this.srcName);
            return false;
        }
...
*/\r\n\r\n", title, message, path) + this.content;
     ...
     */
    test: function () {
        var filter = createNameRouter(arguments);
...
BuildRule.prototype = {
    ...
    getFiles: function () {
        // 特定的规则直接获取对应文件列表。
        if (isSpecifiedName(this.filters)) {
            return this.filters;
        }

        var result = [];
        for (var name in this.builder.files) {
            if (this.match(this.builder.files[name].name) !== null) {
                result.push(name);
            }
        }
        return result;
