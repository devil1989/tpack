...
Builder.prototype = {
     ...
     */
    buildFile: function (name, content) {
        var file = new BuildFile(this, name, content);
    ...
    build: function (clean) {
        ...
        // 3. 执行一次性规则。
        for (var i = 0; i < this.rules.length; i++) {
            if (!this.rules[i].srcNames.length) {
                this.buildFile("#Generated_" + i, "");
            }
        }
    ...
    createFile: function (name, content) {
        var file = new BuildFile(this, "", content);
        file.name = name;
        return file;
    },
    ...
    saveSourceMap: function (sourceMap) {
        ...
        if (!this._hasSourceMap) {
            ...
            this.on('saved', function () {
                ...
                this.builder.createFile(sourceMapOptions.name, JSON.stringify(sourceMapOptions.data)).save();
            });
        }
...
function isSpecifiedName(name) {
    return Array.isArray(name) ? name.every(isSpecifiedName) : typeof name === "string" && !/[\*\?]/.test(name);
}
