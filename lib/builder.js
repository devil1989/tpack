...
function Builder(parentBuilder) {
    if (parentBuilder) {
        ...
        this.warnings = parentBuilder.warnings.slice(0);
...
Builder.prototype = {
    ...
    ignored: function (name) {
        ...
        if (!containsDir(this.destPath, this.srcPath)) {
            ignores = this.ignores.slice(0);
            ignores.push(this.getName(this.destPath));
        }

        // 自动忽略配置文件。
        if (exports.configPath) {
            ignores = ignores || this.ignores.slice(0);
            ignores.push(this.getName(exports.configPath));
        }

        this.ignored = createNameFilter(ignores);
    ...
    processFileWithRule: function (file, rule) {
        ...
        file.srcName && this.debug("> {0}", file.srcName);
    ...
    newContext: function () {
        // 删除之前生成的文件。
        for (var id = 0; id < this._fileId; id++) {
     ...
     * @returns {BuildFile} 返回对应的文件实例。如果文件不存在则自动创建。 
     */
    ...
    addFile: function (name, content) {
        var id = '#' + this._fileId++;
        var file = this.files[id] = this.createFile(id, content);
        file.name = name;
        return file;
    },
    ...
    build: function (clean) {
        ...
        this.errors.length = this.warnings.length = 0;

        // 收集所有文件。
        this.debug("> Collecting Files...", this);
        var files = this.files = { __proto__: null };
        this.walk(function (name) {
            files[name] = this.createFile(name);
        });
        ...
        for (var rules = this.rules, i = 0; i < rules.length; i++) {
            ...
            if (rule.runOnce) {
                var id = '#' + this._fileId++;
                var file = files[id] = this.createFile(id, "");
                this.processFileWithRule(file, rule);
                continue;
            }
            ...
            modules: {
                buildModule: {
                    processRequest: function (context) {
                        ...
                        // 生成当前请求相关的文件。
                        var file = builder.createFile(context.request.path.slice(1));
     ...
     * @example builder.addCdn("assets", "assets");
     */
    ...
    getFiles: function () {
        if (arguments.length === 0) {
            ...
            for (var name in this.files) {
                result.push(this.files[name].srcName);
            }
            return result;
        }
    ...
    getMimeTypeByExt: function (ext) {
        return mimeTypes[ext] || ext.replace('.', 'application/x-');
    }
...
*/\r\n\r\n", title, message, path) + this.content;
    ...
    saveSourceMap: function (sourceMap) {
        ...
        if (!this._hasSourceMap) {
            ...
            this.on('saved', function () {
                ...
                this.builder.addFile(sourceMapOptions.name, sourceMapOptions.content).save();
            });
        }
    ...
    /**
     * 在当前文件内创建引用内容所使用的占位符。
     * @param {Function} callback 获取实际内容的回调函数。
     * @remark
     * 由于发布过程中文件可能发生改变，文件内部的相对路径或内联部分可能因此而失效。
     * 因此为了方便程序处理，在解析文件时将文件内的依赖部分设置为占位符，
     * 在文件真正保存前才替换回相对路径。
     */
    createPlaceholder: function (callback) {

        // 追加占位符处理程序。
        if (!this._placeholderCreated) {
            this._placeholderCreated = true;
            this.on('save', function () {
                this.content = this._parsePlaceholder(this.content);
            });
        }

        // 追加占位符。
        var context = this.builder.getContext();
        var placeholders = context.placeholders || (context.placeholders = []);
        var id = placeholders.length;
        placeholders[id] = callback;
        return "tpack://PH:" + id + "/";
    },
     ...
     * @returns {} 
     */
    _parsePlaceholder: function (content) {
        var placeholders = this.builder.getContext().placeholders || 0;
        var file = this;
        return content.replace(/tpack:\/\/PH:(\d+)\//g, function (all, id) {
            ...
            // 执行占位符程序。
            var result = placeholder.call(file, file);

            // 对占位符解析得到的结果继续解析。
            return file._parsePlaceholder(result);
        });
    },
