...
Builder.prototype = {
    ...
    ignored: function (name) {
		if (!this._ignored) {
			...
			this._ignored = createRouter(ignores);
		}
		return this._ignored(name) !== null;
	},
    ...
    processFile: function (file) {
		...
		for (var i = 0; i < this.rules.length; i++) {
			...
			if (destName !== null) {
				...
				if (this.errors.length > lastErrorCount) {
					return false;
				}
			}
		}
    ...
    processFileWithRule: function (file, rule, destName) {
		...
		for (var i = 0; i < rule.processors.length; i++) {
			...
			if (result !== undefined) {
				if (result instanceof BuildFile) {
					file = result;
				} else {
					file.content = result;
				}
    ...
    watch: function () {
		...
		watcher._isIgnored = function (path, stats) {
			...
			if ((filter && filter.call(builder, name) === null) || builder.ignored(name)) {
				return true;
			}
    ...
    loadIgnoreFile: function (path) {
		IO.readFile(path).split(/[\r\n]/).forEach(function (content) {
			...
			this.ignore.apply(this, createFilter(content));
		}, this);
	},
	
	/**
	 * 确保在一次生成过程中指定的函数仅执行一次。
...
*/\r\n\r\n", title, message, path) + this.content;
    ...
    resolvePath: function (relativePath) {
		...
		return Path.join(this.builder.srcPath, this.srcDirPath, relativePath || "");
	},
...
BuildRule.prototype = {
     ...
     */
    save: function (name) {
		return this.pipe(name ? function (file) {
			var oldDestName = file.destName;
			file.destName = name;
			try {
				file.save();
			} finally {
				file.destName = oldDestName;
			}
		} : function (file) {
			file.save();
		});
